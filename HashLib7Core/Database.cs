...
*/
        ...
        private void WriteHash(string filename, string hashfile, long filesize, DateTime LastModified, bool newRecord)
        {
            ...
            //if (FileExists(f))
            {
                ...
                string sqlInsert = SafeSql("INSERT INTO [dbo].[FileDetail] (Path, Name, Hash, Size, Age, LastModified, LastScanned) VALUES ('{0}', '{1}', '{2}', {3}, {4}, '{5}', getdate())", f.Path, f.Name, hashfile, filesize.ToString(), LastModified.Ticks.ToString(), LastModified.ToString("yyyy-MM-dd HH:mm:ss"));
                ExecuteNonQuery(sqlInsert);
            }
            else
            {
                Config.LogDebugging("Updating record for " + filename);
                string sqlUpdate = SafeSql("UPDATE [dbo].[FileDetail] SET Hash = '{2}', Size = '{3}', Age = '{4}', LastModified = '{5}', LastScanned = getdate(), Fire = NULL, Corruption = NULL, DiskFailure = NULL, Theft = NULL WHERE Path = '{0}' AND Name = '{1}'", f.Path, f.Name, hashfile, filesize.ToString(), LastModified.Ticks.ToString(), LastModified.ToString("yyyy-MM-dd HH:mm:ss"));
